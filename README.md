<div align="center">

# Jonathan Díaz

**Estudiante de Ingeniería en Software · Zacatecas, México**

Construyo cosas para entender cómo funcionan por dentro.
Backend, criptografía y sistemas — con una playlist propia de fondo.

<!-- TODO: reemplaza [usuario] por tus usuarios reales, o borra el badge que no uses -->

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/[Jonathan Diaz])
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:[tu@correo.com])
[![Portfolio](https://img.shields.io/badge/Portfolio-6C5CE7?style=for-the-badge&logo=vercel&logoColor=white)](https://[tu-sitio.com])

</div>

---

## Sobre mí

Estudio Ingeniería de Software en la **Universidad Autónoma de Zacatecas**. Voy en tercer semestre y, aparte de las materias del plan, me metí a **Programación Competitiva** (una materia de quinto) porque me late resolver problemas bajo presión.

Lo que más me mueve es entender las cosas hasta el fondo, no solo hacerlas funcionar. Por eso ando leyendo cómo está construido Bitcoin en C++, estudiando redes por mi cuenta (aunque no venga en mi plan de estudios) y armando mi propia criptomoneda desde cero como proyecto de aprendizaje.

Fuera de la compu, produzco música y coordino un club de conversación en inglés.

```text
📍  Zacatecas, México
🎓  Ing. en Software — UAZ (3er semestre)
🔐  Rumbo a: ciberseguridad, criptografía y sistemas
🎹  También: producción musical (dark pop / indie)
🌱  Aprendiendo: C++, redes, alemán (A1–A2)
```

---

## Stack

**Con lo que trabajo**

![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)

**Herramientas**

![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)
![LaTeX](https://img.shields.io/badge/LaTeX-008080?style=flat-square&logo=latex&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white)

**En proceso**

![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat-square&logo=gnubash&logoColor=white)

<!-- TODO: agrega o quita badges según lo que realmente uses.
     Puedes generar más en https://shields.io o copiar de
     https://github.com/Ileriayo/markdown-badges -->

---

## Proyectos

### 🔗 Criptomoneda desde cero
Proyecto personal para entender Bitcoin a nivel de código. Voy por fases: fundamentos criptográficos → cadena de bloques → minería (Proof of Work) → transacciones y modelo UTXO → red P2P → y al final leer Bitcoin Core directamente.

`Criptografía` `Estructuras de datos` `Redes P2P`

<!-- TODO: cuando tengas el repo, enlázalo aquí -->

---

### ⚡ Programación competitiva
Soluciones a problemas de algoritmos y estructuras de datos. Materia adelantada de quinto semestre.

`Algoritmos` `Estructuras de datos`

<!-- TODO: enlaza aquí tu repo de soluciones si lo tienes público -->

---

## Aprendiendo ahorita

| Tema | Por qué |
|------|---------|
| **C++** | Para poder leer y entender Bitcoin Core |
| **Redes de computadoras** | No viene en mi plan de estudios y es base para ciberseguridad |
| **Criptografía** | Hashing, firmas digitales, curvas elípticas |
| **Linux / terminal** | Mi entorno de trabajo diario |
| **Ciencia de datos** | Me atrae demasiado las ciencias de datos |
| **Alemán (A1–A2)** | Porque sí |

---

## Fuera del código

Produzco música en un espacio entre **dark pop, indie melancólico y urbano latino experimental**. Ableton Live, un MPC Sample y sintes como Vital. Influencias: Álvaro Díaz, NSQK, Latin Mafia, The Neighbourhood, Radiohead.

---

## Idiomas

🇲🇽 Español — nativo · 🇺🇸 Inglés — avanzado (coordino un club de conversación) · 🇩🇪 Alemán — básico (A1–A2)

---

<div align="center">



![Racha](https://streak-stats.demolab.com/?user=[usuario]&theme=tokyonight&hide_border=true&background=0c0e13&ring=8b7cf6&fire=8b7cf6)

</div>

---

<div align="center">
<sub>Siempre construyendo algo. Si algo de aquí te sirve o te da curiosidad, escríbeme.</sub>
</div>
